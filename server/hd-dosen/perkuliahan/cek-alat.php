...
function getDetailMahasiswa($sim_con, $npm){
  $hsl = array();
  $sql = "SELECT TIME(trabsenmhs.TGLTRABSEN) AS TGLTRABSEN, msmhs.NMMHSCMSMHS, msmhs.NIMHSCMSMHS FROM trabsenmhs LEFT JOIN msmhs ON trabsenmhs.NPMTRABSEN = msmhs.NIMHSCMSMHS WHERE trabsenmhs.NPMTRABSEN = '$npm' AND DATE(trabsenmhs.TGLTRABSEN) = '2017-08-26'";
  $hasil = mysqli_query($sim_con, $sql);
  while($row = mysqli_fetch_object($hasil)){
    $hsl = $row;
  }
