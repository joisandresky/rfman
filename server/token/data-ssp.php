...
//data kolom yang akan di tampilkan
    $aColumns = array( 'token_id', 'used', 'rf_id', 'status', 'no_induk', 'keterangan_aksi');

    //primary key
    $sIndexColumn = "token_id";

    //nama table database
    $sTable = "(
        SELECT users_token.token_id as token_id, users_token.used as used, users_token.rf_id as rf_id, users_token.status as status, users_token.no_induk as no_induk, tb_aksi_token.keterangan_aksi as keterangan_aksi FROM users_token LEFT JOIN tb_aksi_token ON users_token.id_aksi = tb_aksi_token.id_aksi
      ) users_token";
    $sOrder = '';
